...
trait Form
{
    ...
    public function createResource(ResourceInput $input)
    {
        ...
        foreach ($fillable as $column) {
            if (array_key_exists($column, $form)) {
                $this->model->$column = is_array($form[$column]) ? null : $form[$column];
            }
        }
    ...
    public function updateResource($resource, ResourceInput $input)
    {
        ...
        foreach ($resource->getFillable() as $column) {
            if (array_key_exists($column, $form)) {
                $resource->$column = is_array($form[$column]) ? null : $form[$column];
            }
        }
    ...
    {
        if (! $id) {
            return app()->abort(400, "Invalid ID");
        }
        $result = $this->model;
